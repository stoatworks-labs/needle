...
namespace
{
constexpr const char* kAppName = "needle";

std::mutex g_mutex;
std::string g_path;
bool g_ready = false;

/// Last message logged under each key, for stateChanged. A handful of keys for
/// the life of the process -- there is no eviction because there is nothing to
/// evict.
std::map< std::string, std::string > g_lastState;
...
{
	const std::string override_ = environmentVariable( "NEEDLE_LOG_DIR" );
	if( !override_.empty() )
		return override_;
...
void stateChanged( const std::string& key, const std::string& message )
{
	{
		std::lock_guard<std::mutex> lock( g_mutex );
		auto it = g_lastState.find( key );
		if( it != g_lastState.end() && it->second == message )
			return;
		g_lastState[ key ] = message;
	}
	info( message );
}
